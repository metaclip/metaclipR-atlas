##     metaclipcc.File METACLIP description of an Atlas intermediate product (netCDF files)
##
...
#' @title Directed metadata graph construction for AR6 Atlas Intermediate product
#' @description Build a directed metadata graph describing an intermediate product of the AR6 Interactive Atlas
#' @param project Project. Default to \code{"CMIP6"}.
...
metaclipcc.File <- function(project = "CMIP5",
                            variable = NULL,
                            climate.index = NULL,
                            experiment,
                            test.mode = FALSE) {
    ...
    bias.adj.method = NULL # set to ISIMIP3 later if required
    season = 1:12
    # time.res.orig <- "P1D"
    # ***********************
    ...
    ## TODO revisar periodos historicos
    if (project == "CMIP6") {
        baseline <- "1850-2014"
        future.period <- "2015-2100"
    } else {
        baseline <- "1850-2005"
        future.period <- "2006-2100"
    }
    fill.period <- NULL
    ...
    # } else {
    #
    #     hist.period <- c(1850, 2005)
    #     fill.period <- c(2006, 2014)
    #
    ...
    for (x in iter) {
        ...
        if (isTRUE(variable %in% c("tos", "ph", "siconc"))) {
    ...
    if (length(graph.list) > 1L) {
        descr <- "The multi-model ensemble is built by joining each individual model along the new dimension \'member\'"
        graph <- metaclipR.Ensemble(graph.list = graph.list,
                                    disable.command = TRUE,
                                    dc.description = descr)
    } else {## It can happen that one single model remains (e.g. CMIP6 SSP245 +4degC GWL)
